...
volatile enum sha_algos opt_algo = ALGO_SCRYPT_JANE;
int opt_n_threads = 0;
int gpu_threads = 1;
...
int opt_reserve_ram = 600; // System RAM to reserve in MB (default: 600 MB)
...
"			hmq1725     Doubloons / Espers\n\
  ...
  -b, --api-bind=port   IP:port for the miner API (default: 127.0.0.1:4068), 0 disabled\n\
      ...
      --reserve-ram=N   Amount of system RAM to reserve in MB when --use-system-ram is enabled (default: 600)\n"
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (pool->type & POOL_STRATUM) {
		...
		// scrypt-jane nonce/ntime encoding
...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	/* Generate merkle root - scrypt-jane uses sha256d */
	...
	// scrypt-jane target calculation
...
static void *miner_thread(void *userdata)
{
	...
	while (!abort_flag) {
		...
		// scrypt-jane: adjust nonce position based on version
		if (nVersion >= 7)
		{
			// modify nNonce position, change to adapt with 64-bit nTime
		...
		/* scan nonces for a proof-of-work hash - only scrypt-jane supported */
		...
		// scrypt-jane uses pdata[21] for nonces
...
void parse_arg(int key, char *arg)
{
	...
	case 'a': /* --algo */
		...
		if (i >= 0)
			...
			applog(LOG_ERR, "Unknown algo parameter '%s' - only scrypt-jane is supported", arg);
		...
		if (!opt_nfactor) {
			opt_nfactor = 14; // default for scrypt-jane
		}
